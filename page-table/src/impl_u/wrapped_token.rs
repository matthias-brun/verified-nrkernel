                        ...
                        GPDE::Directory { addr: l2_daddr, .. } => {
                            ...
                            match l2e@ {
                                GPDE::Page { .. } => {
                                    ...
                                    assert(iset![l3_base as nat].contains(l3_base as nat));
                                    assert(l3_base == vaddr);
                                    assert(interp_l2.interp_of_entry(l2_bidx as nat).contains_key(vaddr as nat));
...
impl WrappedMapToken {
            ...
            final(tok).inv(),
    {
        ...
        proof {
            ...
            let step = os::Step::MapOpStutter { core, paddr: addr, value };
            assert(os::next_step(tok.tok.consts(), tok.tok.st(), post, step, RLbl::Tau));
            tok.tok.register_internal_step_mmu(&mut mmu_tok, post, step);
            os_invariant::next_preserves_inv(tok.tok.consts(), state1, tok.tok.st(), RLbl::Tau);
        }
            ...
            final(tok).inv(),
    {
        ...
        proof {
            ...
            let step = os::Step::MapOpChange { core, paddr: addr, value };
            assert(os::next_step(tok.tok.consts(), tok.tok.st(), post, step, RLbl::Tau));
            tok.tok.register_internal_step_mmu(&mut mmu_tok, post, step);
            os_invariant::next_preserves_inv(tok.tok.consts(), state1, tok.tok.st(), RLbl::Tau);
        }
            ...
            final(tok).inv(),
    {
        ...
        proof {
            ...
            let step = os::Step::Allocate { core, res: osext_tok.lbl()->Allocate_res };
            assert(os::next_step(tok.tok.consts(), tok.tok.st(), post, step, RLbl::Tau));
        ...
        }) ==> exists|pt| PT::inv_and_nonempty(wtok, pt),
{
    ...
    proof {
        ...
        let step = os::Step::MapOpStart { core };
...
impl WrappedUnmapToken {
            ...
            final(tok).inv(),
    {
        ...
        proof {
            ...
            let step = os::Step::Deallocate { core, reg: region@ };
            assert(os::next_step(tok.tok.consts(), tok.tok.st(), post, step, RLbl::Tau));
            tok.tok.register_internal_step_osext(&mut osext_tok, post, step, RLbl::Tau);
            os_invariant::next_preserves_inv(tok.tok.consts(), state1, tok.tok.st(), RLbl::Tau);
        }
...
impl WrappedProtectToken {
   ...
   pub closed spec fn view(&self) -> WrappedTokenView {
    ...
    /// Completes the remaining protect transitions and performs a shootdown if necessary.
    #[verifier(spinoff_prover)]
            ...
            rtok@.steps_taken().last()->ProtectEnd_result == (if shootdown is Yes { Ok(()) } else { Err(()) }),
    {
        ...
        if let DoShootdown::Yes { vaddr } = shootdown {
            ...
            proof {
                ...
                assert(os::step_ProtectOpFail(tok.tok.consts(), tok.tok.st(), post, core, RLbl::Tau));
                let step = os::Step::ProtectOpFail { core };
                assert(os::next_step(tok.tok.consts(), tok.tok.st(), post, step, RLbl::Tau));
                tok.tok.register_internal_step(post, step);
                os_invariant::next_preserves_inv(tok.tok.consts(), state1, tok.tok.st(), RLbl::Tau);
                let ghost state2 = tok.tok.st();
        ...
        proof {
            ...
            let cs = tok.tok.st().core_states[core];
            let result = if cs is ProtectShootdownWaiting { Ok(()) } else { Err(()) };
            ...
            assert(os_ext::step_ReleaseLock(tok.tok.st().os_ext, post.os_ext, tok.tok.consts().common, osext_tok.lbl()));
            let lbl = RLbl::ProtectEnd { thread_id: tok.tok.thread(), vaddr, result };
        ...
        }) ==> exists|pt| PT::inv_and_nonempty(wtok, pt),
{
    ...
    proof {
        ...
        let step = os::Step::UnmapStart { core };
        assert(os::next_step(tok.consts(), tok.st(), post, step, lbl));
        tok.register_external_step(post, step, lbl);
        let state3 = tok.st();
        os_invariant::next_preserves_inv(tok.consts(), state2, state3, lbl);
    ...
    proof {
        ...
        let step = os::Step::UnmapOpStart { core };
        assert(os::next_step(tok.consts(), tok.st(), post, step, RLbl::Tau));
        tok.register_internal_step_osext(&mut osext_tok, post, step, RLbl::Tau);
        os_invariant::next_preserves_inv(tok.consts(), state5, tok.st(), RLbl::Tau);
    }
