...
use crate::spec_t::mmu::defs::{
    bit, bitmask_inc, x86_arch_exec, Flags, MemRegion, MemRegionExec, PageTableEntryExec, L0_ENTRY_SIZE, L1_ENTRY_SIZE, L2_ENTRY_SIZE, L3_ENTRY_SIZE, MAX_PHYADDR, MAX_PHYADDR_WIDTH, PAGE_SIZE, PKEY_MAX, PTE, WORD_SIZE, X86_NUM_ENTRIES, X86_NUM_LAYERS
};
...
use crate::spec_t::mmu::translation::{
    GPDE, MASK_ADDR, MASK_DIR_ADDR, MASK_FLAG_P, MASK_FLAG_PCD, MASK_FLAG_PWT, MASK_FLAG_RW, MASK_FLAG_US, MASK_FLAG_XD, MASK_L1_PG_ADDR, MASK_L1_PG_FLAG_PS, MASK_L2_PG_ADDR, MASK_L3_PG_ADDR, MASK_L3_PG_FLAG_PAT, MASK_NEG_DIRTY_ACCESS, MASK_NEG_PKEY, MASK_NEG_PROT_FLAGS, MASK_PG_FLAG_PAT, MASK_PKEY, MASK_PROT_ALL, MASK_NEG_PROT_ALL, MASK_PKEY_MAX, SHIFT_PKEY, MASK_L2_PG_FLAG_PS, PDE
};
#[cfg(verus_keep_ghost)]
use crate::extra;
...
broadcast proof fn lemma_union_empty<A>(s: ISet<A>)
    ensures #[trigger] s.union(iset![]) == s,
{
    assert(s.union(iset![]) =~= s);
}

broadcast proof fn lemma_difference_empty<A>(s: ISet<A>)
    ensures #[trigger] s.difference(iset![]) == s,
{
    assert(s.difference(iset![]) =~= s);
}
...
impl PDE {
            ...
            }),
    {
        ...
        let or7 = if is_page         { ((pkey as usize) << SHIFT_PKEY) } else { 0 };
        let or8 = if disable_execute { MASK_FLAG_XD }  else { 0 };
        ...
        assert(forall|a:usize,i:usize| #![auto] i > 51 && (a & bitmask_inc!(mw,51usize)    == 0) ==> ((a | bit!(i)) & bitmask_inc!(mw,51usize) == 0)) by (bit_vector)
            requires mw <= 52;
        assert(address & bitmask_inc!(mw, SHIFT_PKEY - 1) == 0) by (bit_vector)
            requires
    ...
    pub proof fn lemma_new_entry_addr_mask_is_address(
        ...
        ensures
            ({ let e = address
                ...
                | if disable_execute { MASK_FLAG_XD }  else { 0 };
               &&& e & MASK_ADDR == address
               &&& (is_page ==> ((e >> SHIFT_PKEY) & MASK_PKEY_MAX) == pkey)
            ...
            }),
    {
        ...
        assert((pkey as usize) & 0x0000_0000_0000_000f == (pkey as usize)) by (bit_vector)
            requires pkey <= 0x0000_0000_0000_000f;

        assert(((pkey as usize) << SHIFT_PKEY) & MASK_PKEY == (pkey as usize) << SHIFT_PKEY) by (bit_vector)
            requires pkey <= 0x0000_0000_0000_000f;
        ...
        assert(forall|a:usize,i:usize| #![auto] i < 12 ==> a & bitmask_inc!(12usize, sub(mw, 1)) == (a | bit!(i)) & bitmask_inc!(12usize, sub(mw, 1))) by (bit_vector)
            requires 32 <= mw <= 52;
        assert(forall|a:usize,i:usize| #![auto] i < 12 ==> a & bitmask_inc!(SHIFT_PKEY, add(SHIFT_PKEY, 3)) == (a | bit!(i))  & bitmask_inc!(SHIFT_PKEY, add(SHIFT_PKEY, 3))) by (bit_vector);
        ...
        assert(forall|a:usize,i:usize| #![auto] i < SHIFT_PKEY ==> a & bitmask_inc!(SHIFT_PKEY, add(SHIFT_PKEY, 3)) == (a | bit!(i))  & bitmask_inc!(SHIFT_PKEY, add(SHIFT_PKEY, 3))) by (bit_vector);
        assert(forall|a:usize,i:usize| #![auto] i > SHIFT_PKEY + 3 ==> a & bitmask_inc!(SHIFT_PKEY, add(SHIFT_PKEY, 3)) == (a | bit!(i))  & bitmask_inc!(SHIFT_PKEY, add(SHIFT_PKEY, 3))) by (bit_vector);

        assert(forall|a:usize,i:usize| #![auto] i < 12 ==> a & bitmask_inc!(12usize, sub(mw, 1)) == a ==> a & bit!(i) == 0) by (bit_vector)
            requires 32 <= mw <= 52;
       assert(forall|a:usize,i:usize| #![auto] i < 12 ==> a & bitmask_inc!(SHIFT_PKEY, add(SHIFT_PKEY, 3)) == a ==> a & bit!(i) == 0) by (bit_vector);

        assert(forall|a:usize,i:usize| #![auto] i > SHIFT_PKEY + 3 ==> a & bitmask_inc!(SHIFT_PKEY, add(SHIFT_PKEY, 3)) == a ==> a & bit!(i) == 0) by (bit_vector);
        ...
        assert(bitmask_inc!(12usize, sub(mw, 1)) & bitmask_inc!(SHIFT_PKEY, add(SHIFT_PKEY, 3)) == 0) by (bit_vector)
            requires 32 <= mw <= 52;


        assert(forall |i:usize| i < SHIFT_PKEY ==> bit!(i) & bitmask_inc!(SHIFT_PKEY, add(SHIFT_PKEY, 3)) == 0) by (bit_vector);
        assert(bitmask_inc!(12usize, sub(mw, 1)) & bitmask_inc!(SHIFT_PKEY, add(SHIFT_PKEY, 3)) == 0) by (bit_vector)
            requires 32 <= mw <= 52;

        assert(address & bitmask_inc!(12usize, sub(mw, 1)) == address);

        assert(e & MASK_FLAG_P == MASK_FLAG_P) by (bit_vector)
            requires
                e == address | or1 | or2 | or3 | or4 | or5 | or6 | or7 | or8,
                or1 == MASK_FLAG_P,
                address & bitmask_inc!(12usize, sub(mw, 1)) == address,
                32 <= mw <= 52;

        assert(e & bitmask_inc!(12usize, sub(mw, 1)) == address) by (bit_vector)
            requires
                e == address | or1 | or2 | or3 | or4 | or5 | or6 | or7 | or8,
                or1 == MASK_FLAG_P,
                or2 == if is_page         { bit!(7) }       else { 0 },
                ...
                or7 == if is_page         { ((pkey as usize) << SHIFT_PKEY) } else { 0 },
        ...
        assert((is_page ==> ((e >> SHIFT_PKEY) & MASK_PKEY_MAX) == pkey)) by {
            if is_page {
                assert(or7 == ((pkey as usize) << SHIFT_PKEY));
                assert(e & MASK_PKEY == ((pkey as usize) << SHIFT_PKEY)) by (bit_vector)
                    requires
                        e == address | or1 | or2 | or3 | or4 | or5 | or6 | or7 | or8,
                        or1 == MASK_FLAG_P,
                        or2 == bit!(7),
                        ...
                        pkey < PKEY_MAX,
                        32 <= mw <= 52;
                assert(((e >> SHIFT_PKEY) & MASK_PKEY_MAX) == pkey) by (bit_vector)
                    requires
                        e & MASK_PKEY == ((pkey as usize) << SHIFT_PKEY),
                        pkey < PKEY_MAX;
            }
        }

        assert(e & MASK_FLAG_RW == or3) by (bit_vector)
            requires
                e == address | or1 | or2 | or3 | or4 | or5 | or6 | or7 | or8,
                or1 == MASK_FLAG_P,
                or2 == if is_page         { bit!(7) }       else { 0 },
        ...
        assert(e & MASK_FLAG_US == or4) by (bit_vector)
            requires
                e == address | or1 | or2 | or3 | or4 | or5 | or6 | or7 | or8,
                or1 == MASK_FLAG_P,
                or2 == if is_page         { bit!(7) }       else { 0 },
        ...
        assert(e & MASK_FLAG_PWT == or5) by (bit_vector)
            requires
                e == address | or1 | or2 | or3 | or4 | or5 | or6 | or7 | or8,
                or1 == MASK_FLAG_P,
                or2 == if is_page         { bit!(7) }       else { 0 },
        ...
        assert(e & MASK_FLAG_PCD == or6) by (bit_vector)
            requires
                e == address | or1 | or2 | or3 | or4 | or5 | or6 | or7 | or8,
                or1 == MASK_FLAG_P,
                or2 == if is_page         { bit!(7) }       else { 0 },
        ...
        assert(e & MASK_FLAG_XD == or8) by (bit_vector)
            requires
                e == address | or1 | or2 | or3 | or4 | or5 | or6 | or7 | or8,
                or1 == MASK_FLAG_P,
                or2 == if is_page         { bit!(7) }       else { 0 },
        ...
        assert((is_page && layer == 2 ==> e & MASK_PG_FLAG_PAT == 0)) by {
            if is_page && layer == 2 {
        ...
        assert(e & bit!(5) == 0) by (bit_vector)
            requires
                e == address | or1 | or2 | or3 | or4 | or5 | or6 | or7 | or8,
                or1 == MASK_FLAG_P,
                or2 == if is_page         { bit!(7) }       else { 0 },
                ...
                pkey < PKEY_MAX,
                32 <= mw <= 52;
        assert(e & bit!(6) == 0) by (bit_vector)
            requires
                e == address | or1 | or2 | or3 | or4 | or5 | or6 | or7 | or8,
                or1 == MASK_FLAG_P,
                or2 == if is_page         { bit!(7) }       else { 0 },
                ...
                pkey < PKEY_MAX,
                32 <= mw <= 52;
        assert((e & bit!(7) == bit!(7)) <==> is_page) by (bit_vector)
            requires
                e == address | or1 | or2 | or3 | or4 | or5 | or6 | or7 | or8,
                or1 == MASK_FLAG_P,
                or2 == if is_page         { bit!(7) }       else { 0 },
    ...
    pub fn new_page_entry(layer: usize, pte: &PageTableEntryExec) -> (r: Self)
        ...
        ensures
            ...
            r.entry & bit!(6) == 0,
            r.entry & bit!(7) == bit!(7),
            r@->Page_pkey == pte.pkey,
    {
        Self::new_entry(layer, pte.frame.base, true, pte.flags.is_writable, pte.flags.is_supervisor, false, false, pte.flags.disable_execute, pte.pkey)
    }

    pub exec fn set_pkey(val: usize, pkey: u8) -> (res: usize)
        requires pkey < PKEY_MAX
        ensures
            PDE::spec_extract_pkey(res) == pkey,
            res & MASK_NEG_PKEY == val & MASK_NEG_PKEY
    {
        let r1 = (val & MASK_NEG_PKEY) | (((pkey as usize) & MASK_PKEY_MAX) << SHIFT_PKEY);
        proof {
            assert(r1 & MASK_NEG_PKEY == val & MASK_NEG_PKEY) by (bit_vector)
                requires pkey < PKEY_MAX, r1 == (val & MASK_NEG_PKEY) | (((pkey as usize) & MASK_PKEY_MAX) << SHIFT_PKEY);
            PDE::lemma_insert_extract_equal(val, pkey);
        }
        r1

    }

    pub exec fn set_flags(val: usize, flags: &Flags) -> (res: usize)
        ensures
            res & MASK_NEG_PROT_FLAGS == val & MASK_NEG_PROT_FLAGS,
            (res & MASK_FLAG_US) == MASK_FLAG_US <==> !flags.is_supervisor,
            (res & MASK_FLAG_RW) == MASK_FLAG_RW <==> flags.is_writable,
            (res & MASK_FLAG_XD) == MASK_FLAG_XD <==> flags.disable_execute,
    {
        let r1 = (val & MASK_NEG_PROT_FLAGS)
            | if flags.is_writable     { MASK_FLAG_RW }  else { 0 }
            | if flags.is_supervisor   { 0 }             else { MASK_FLAG_US }
            | if flags.disable_execute { MASK_FLAG_XD }  else { 0 };
        proof {
            let is_writable = flags.is_writable;
            let is_supervisor = flags.is_supervisor;
            let disable_execute = flags.disable_execute;
            assert(r1 & MASK_NEG_PROT_FLAGS == val & MASK_NEG_PROT_FLAGS
                 && ((r1 & MASK_FLAG_US) == MASK_FLAG_US <==> !is_supervisor)
                 && ((r1 & MASK_FLAG_RW) == MASK_FLAG_RW <==> is_writable)
                 && ((r1 & MASK_FLAG_XD) == MASK_FLAG_XD <==> disable_execute)
            ) by (bit_vector)
                requires r1 == (val & MASK_NEG_PROT_FLAGS)
                            | if is_writable     { MASK_FLAG_RW }  else { 0 }
                            | if is_supervisor   { 0 }             else { MASK_FLAG_US }
                            | if disable_execute { MASK_FLAG_XD }  else { 0 };
    ...
    pub exec fn set_prot(val: usize, flags: &Flags, pkey: u8) -> (res: usize)
        requires pkey < PKEY_MAX
        ensures
            res & MASK_NEG_PROT_ALL == val & MASK_NEG_PROT_ALL,
            PDE::spec_extract_pkey(res) == pkey,
            (res & MASK_FLAG_US) == MASK_FLAG_US <==> !flags.is_supervisor,
            (res & MASK_FLAG_RW) == MASK_FLAG_RW <==> flags.is_writable,
            (res & MASK_FLAG_XD) == MASK_FLAG_XD <==> flags.disable_execute,
    {
        let r1 = Self::set_flags(val, flags);
        let r2 = Self::set_pkey(r1, pkey);
        proof {
            assert((r2 & MASK_FLAG_US) == (r1 & MASK_FLAG_US)) by (bit_vector)
                requires r1 & MASK_NEG_PKEY == r2 & MASK_NEG_PKEY;
            assert((r2 & MASK_FLAG_RW) == (r1 & MASK_FLAG_RW)) by (bit_vector)
                requires r1 & MASK_NEG_PKEY == r2 & MASK_NEG_PKEY;
            assert((r2 & MASK_FLAG_XD) == (r1 & MASK_FLAG_XD)) by (bit_vector)
                requires r1 & MASK_NEG_PKEY == r2 & MASK_NEG_PKEY;
            assert(r2 & MASK_NEG_PROT_ALL == val & MASK_NEG_PROT_ALL) by (bit_vector)
                requires
                    val & MASK_NEG_PROT_FLAGS == r1 & MASK_NEG_PROT_FLAGS,
                    r1 & MASK_NEG_PKEY == r2 & MASK_NEG_PKEY;

        }
        r2
    }

    pub fn change_page_permissions(&self, flags: &Flags, pkey: u8) -> (r: Self)
        requires
            self.layer@ <= 3,
            pkey < PKEY_MAX,
            self@ is Page,
            self.hp_pat_is_zero(),
            self.entry & bit!(7usize) == bit!(7usize),
        ensures
            r@ is Page,
            r.entry & MASK_NEG_PROT_ALL == self.entry & MASK_NEG_PROT_ALL,
            r@->Page_RW <==> flags.is_writable,
            r@->Page_US <==> !flags.is_supervisor,
            r@->Page_XD <==> flags.disable_execute,
            r.layer == self.layer,
            r@->Page_addr == self@->Page_addr,
            r@->Page_pkey == pkey,
            r.hp_pat_is_zero(),
            r.entry & bit!(7usize) == bit!(7usize),
    {
        let old_entry = self.entry;
        let new_entry = Self::set_prot(old_entry, flags, pkey);
        let new_pde = PDE {
            entry: new_entry,
            layer: self.layer,
        };
        ...
        proof {
            ...
            assert(old_entry & MASK_NEG_DIRTY_ACCESS & MASK_NEG_PROT_ALL == new_entry & MASK_NEG_DIRTY_ACCESS & MASK_NEG_PROT_ALL) by (bit_vector)
                requires old_entry & MASK_NEG_PROT_ALL == new_entry & MASK_NEG_PROT_ALL;

            PDE::lemma_view_unchanged_prot_flags(*self, new_pde);

            assert(new_entry & bit!(7) == old_entry & bit!(7)) by (bit_vector)
                requires old_entry & MASK_NEG_PROT_ALL == new_entry & MASK_NEG_PROT_ALL;

            assert(new_entry & MASK_PG_FLAG_PAT == old_entry & MASK_PG_FLAG_PAT) by (bit_vector)
                requires old_entry & MASK_NEG_PROT_ALL == new_entry & MASK_NEG_PROT_ALL;
        }
        ...
        //     assert(forall|a: usize| #![auto] a == a | 0) by (bit_vector);
        //     assert(forall|a: usize, b: usize| a & b & b == a & b) by (bit_vector);
        //     assert(forall|a: usize| #[trigger] ((a | MASK_FLAG_RW) & MASK_NEG_PROT_FLAGS) == a & MASK_NEG_PROT_FLAGS) by (bit_vector);
        //     assert(forall|a: usize| #[trigger] ((a | MASK_FLAG_US) & MASK_NEG_PROT_FLAGS) == a & MASK_NEG_PROT_FLAGS) by (bit_vector);
        //     assert(forall|a: usize| #[trigger] ((a | MASK_FLAG_XD) & MASK_NEG_PROT_FLAGS) == a & MASK_NEG_PROT_FLAGS) by (bit_vector);
        //     assert(forall|a: usize, i: usize| i < 63 && i != 1 && i != 2
        //         ==> #[trigger] (a & MASK_NEG_PROT_FLAGS & bit!(i)) == a & bit!(i)) by (bit_vector);
        //     assert(forall|a: usize, i: usize| #[trigger] ((a | bit!(i)) & bit!(i)) == bit!(i)) by (bit_vector);
        //     assert(forall|a: usize| #[trigger] ((a & MASK_NEG_PROT_FLAGS) & MASK_FLAG_RW) == 0) by (bit_vector);
        //     assert(forall|a: usize| #[trigger] ((a & MASK_NEG_PROT_FLAGS) & MASK_FLAG_US) == 0) by (bit_vector);
        //     assert(forall|a: usize| #[trigger] ((a & MASK_NEG_PROT_FLAGS) & MASK_FLAG_XD) == 0) by (bit_vector);


        //     assert(forall|a: usize| #[trigger] ((a | MASK_FLAG_RW) & MASK_NEG_PROT_ALL) == a & MASK_NEG_PROT_ALL) by (bit_vector);
        //     assert(forall|a: usize| #[trigger] ((a | MASK_FLAG_US) & MASK_NEG_PROT_ALL) == a & MASK_NEG_PROT_ALL) by (bit_vector);
        //     assert(forall|a: usize| #[trigger] ((a | MASK_FLAG_XD) & MASK_NEG_PROT_ALL) == a & MASK_NEG_PROT_ALL) by (bit_vector);
        //     assert(forall|a: usize, i: usize| i < 59 && i != 1 && i != 2
        //         ==> #[trigger] (a & MASK_NEG_PROT_ALL & bit!(i)) == a & bit!(i)) by (bit_vector);
        //     assert(forall|a: usize, i: usize| #[trigger] ((a | bit!(i)) & bit!(i)) == bit!(i)) by (bit_vector);
        //     assert(forall|a: usize| #[trigger] ((a & MASK_NEG_PROT_ALL) & MASK_FLAG_RW) == 0) by (bit_vector);
        //     assert(forall|a: usize| #[trigger] ((a & MASK_NEG_PROT_ALL) & MASK_FLAG_US) == 0) by (bit_vector);
        //     assert(forall|a: usize| #[trigger] ((a & MASK_NEG_PROT_ALL) & MASK_FLAG_XD) == 0) by (bit_vector);



        //     assert(forall|a: usize| #[trigger] ((a | MASK_PKEY) & MASK_NEG_PKEY) == a & MASK_NEG_PKEY) by (bit_vector);
        //     assert(forall|a: usize, i: usize| i < 59
        //         ==> #[trigger] (a & MASK_NEG_PKEY & bit!(i)) == a & bit!(i)) by (bit_vector);
        //     assert(forall|a: usize, i: usize| 62 < i < 64
        //         ==> #[trigger] (a & MASK_NEG_PKEY & bit!(i)) == a & bit!(i)) by (bit_vector);
        //     assert(forall|a: usize| #[trigger] ((a & MASK_NEG_PKEY) & MASK_PKEY) == 0) by (bit_vector);

        //     let old_entry = self.entry;
        //     assert(old_entry & MASK_L1_PG_FLAG_PS == MASK_L1_PG_FLAG_PS) by (bit_vector)
        //         requires old_entry & bit!(7usize) == bit!(7usize);

        //     assert(new_entry & MASK_L1_PG_FLAG_PS == MASK_L1_PG_FLAG_PS & old_entry) by (bit_vector)
        //         requires new_entry & MASK_NEG_PROT_ALL == old_entry & MASK_NEG_PROT_ALL;


        //     assert(forall|a: usize, i: usize, j: usize| i != j
        //         ==> #[trigger] ((a | bit!(i)) & bit!(j)) == a & bit!(j)) by (bit_vector);

        //     assert(forall|a: usize, i1: usize, i2: usize| 2 < i1 <= i2 < 59 ==>
        //         #[trigger] (a & MASK_NEG_PROT_FLAGS & bitmask_inc!(i1, i2))
        //             == a & bitmask_inc!(i1, i2)) by (bit_vector);
        //     assert(forall|a: usize, i1: usize, i2: usize| 2 < i1 <= i2 < 59 ==>
        //         #[trigger] ((a | MASK_FLAG_RW) & bitmask_inc!(i1, i2))
        //             == a & bitmask_inc!(i1, i2)) by (bit_vector);
        //     assert(forall|a: usize, i1: usize, i2: usize| 2 < i1 <= i2 < 59 ==>
        //         #[trigger] ((a | MASK_FLAG_US) & bitmask_inc!(i1, i2))
        //             == a & bitmask_inc!(i1, i2)) by (bit_vector);
        //     assert(forall|a: usize, i1: usize, i2: usize| 2 < i1 <= i2 < 59 ==>
        //         #[trigger] ((a | MASK_FLAG_XD) & bitmask_inc!(i1, i2))
        //             == a & bitmask_inc!(i1, i2)) by (bit_vector);

        //     assert(forall|a: usize, i1: usize, i2: usize| 0 < i1 <= i2 < 59 ==>
        //         #[trigger] ((a | MASK_PKEY) & bitmask_inc!(i1, i2))
        //             == a & bitmask_inc!(i1, i2)) by (bit_vector);
        //     assert(forall|a: usize, i1: usize, i2: usize| 62 < i1 <= i2 < 63 ==>
        //         #[trigger] ((a | MASK_PKEY) & bitmask_inc!(i1, i2))
        //             == a & bitmask_inc!(i1, i2)) by (bit_vector);

        //     assert(bitmask_inc!(52, 51) == 0) by (bit_vector);
        //     assert(forall|a: usize| a & 0 == 0) by (bit_vector);

        //     assert(forall|a: usize, i: usize| i < 64 && (a & bit!(i) == 0) ==> a & bit!(i) != bit!(i)) by (bit_vector);

        //     assert(new_pde.all_mb0_bits_are_zero()) by {
        //         reveal(PDE::all_mb0_bits_are_zero);
        //     };
        // }
        new_pde
    ...
    pub fn new_entry(
        ...
        ensures
            r.all_mb0_bits_are_zero(),
            if is_page { r@ is Page && r@->Page_addr == address && r@->Page_pkey == pkey} else { r@ is Directory && r@->Directory_addr == address},
            r.hp_pat_is_zero(),
            r.entry & bit!(5) == 0,
            r.entry & bit!(6) == 0,
            r.layer@ == layer,
            r.entry & MASK_ADDR == address,
            is_page ==> ((r.entry >> SHIFT_PKEY) & MASK_PKEY_MAX) == pkey,
            r.entry & MASK_FLAG_P == MASK_FLAG_P,
            (r.entry & bit!(7) == bit!(7)) <==> is_page,
            ...
            (r.entry & MASK_FLAG_XD == MASK_FLAG_XD) <==> disable_execute,
    {
        ...
        proof {
            PDE::lemma_new_entry_addr_mask_is_address(layer, address, is_page, is_writable, is_supervisor, is_writethrough, disable_cache, disable_execute, pkey);
            PDE::lemma_new_entry_mb0_bits_are_zero(layer, address, is_page, is_writable, is_supervisor, is_writethrough, disable_cache, disable_execute, pkey);
            if is_page {
...
pub struct PTDir {
    ...
    /// TODO(MB): This should probably just be computed. Don't quite recall why I ended up making
    /// it a field.
    pub used_regions: ISet<MemRegion>,
}
...
use crate::spec_t::mmu::defs::PKEY_MAX;
    ...
    decreases X86_NUM_LAYERS - layer
{
    ...
    &&& pages_have_bit7(tok, pt, layer, ptr)
}
...
/// Writes during a protect phase can only be to locations where the current value has bit 7 set.
pub open spec fn pages_have_bit7(tok: WrappedTokenView, pt: PTDir, layer: nat, ptr: usize) -> bool {
    forall|i: nat| i < X86_NUM_ENTRIES ==>
        #[trigger] entry_at_spec(tok, pt, layer, ptr, i)@ is Page
        ==> entry_at_spec(tok, pt, layer, ptr, i).entry & bit!(7usize) == bit!(7usize)
}
...
pub open spec fn interp_to_l0(tok: WrappedTokenView, pt: PTDir) -> IMap<nat, PTE> {
...
spec fn map_builder_pre(tok_old: WrappedTokenView, pt_old: PTDir, tok_new: WrappedTokenView, pt_new: PTDir, layer: nat, ptr: usize, new_regions: ISet<MemRegion>) -> bool {
...
fn map_frame_aux(
    ...
    Ghost(rebuild_root_pt): Ghost<spec_fn(PTDir, ISet<MemRegion>) -> PTDir>,
) -> (res: Result<Ghost<(PTDir,ISet<MemRegion>)>,()>)
    requires
        old(tok).inv(),
        pte.wf(),
        !old(tok)@.change_made,
        ...
        // we need to prove that the overall page table changes in the expected way, not just the
        // subtree we're currently in. Hence, rebuild_root_pt allows us to basically reconstruct
        // the root PTDir and do the framing proofs to obtain invariant and interpretation of the
        // overall page table from the changes to the individual subtree.
               ...
               ==> {
                   ...
                   &&& interp_at(tok_new, pt_new, layer as nat, ptr, base as nat).interp()
                        ...
                        ==> interp_to_l0(tok_new, rebuild_root_pt(pt_new, new_regions))
                                == interp_to_l0(old(tok)@, rebuild_root_pt(pt, iset![]))
                   &&& interp_at(tok_new, pt_new, layer as nat, ptr, base as nat).interp()
                           === interp_at(old(tok)@, pt, layer as nat, ptr, base as nat).interp().insert(vaddr as nat, pte@)
                        ==> interp_to_l0(tok_new, rebuild_root_pt(pt_new, new_regions))
                                === interp_to_l0(old(tok)@, rebuild_root_pt(pt, iset![])).insert(vaddr as nat, pte@)
        }
    ensures
        final(tok).inv(),
    ...
    if entry.is_present() {
        if entry.is_dir(layer) {
            if x86_arch_exec.entry_size(layer) == pte.frame.size {
                ...
                assert(candidate_mapping_overlaps_existing_vmem(interp_to_l0(tok@, rebuild_root_pt(pt, iset![])), vaddr as nat, pte@));
                Err(())
            } else {
                let dir_addr = entry.address();
                assert(pt.entries[idx as int] is Some);
                ...
                let ghost rebuild_root_pt_inner = |pt_new_inner: PTDir, new_regions: ISet<MemRegion>| {
                    let pt_new = PTDir {
                ...
                assert(pt.entries =~= PTDir {
                    ...
                    used_regions: pt.used_regions.union(iset![]),
                    ..pt
                }.entries);
                assert(forall|s: ISet<MemRegion>| s.union(iset![]) =~= s);
                assert(rebuild_root_pt_inner(dir_pt, iset![]) == rebuild_root_pt(pt, iset![]));
                   ...
                   implies {
                       ...
                       &&& interp_at(tok_new, pt_new_inner, layer as nat + 1, dir_addr, entry_base as nat).interp()
                            ...
                            ==> interp_to_l0(tok_new, rebuild_root_pt_inner(pt_new_inner, new_regions))
                                    == interp_to_l0(tok@, rebuild_root_pt_inner(dir_pt, iset![]))
                       &&& interp_at(tok_new, pt_new_inner, layer as nat + 1, dir_addr, entry_base as nat).interp()
                               === interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat).interp().insert(vaddr as nat, pte@)
                            ==> interp_to_l0(tok_new, rebuild_root_pt_inner(pt_new_inner, new_regions))
                                    === interp_to_l0(tok@, rebuild_root_pt_inner(dir_pt, iset![])).insert(vaddr as nat, pte@)
                       }
                by {
                           ...
                           == interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat).interp()
                    {
                        ...
                        assert(interp_to_l0(tok_new, rebuild_root_pt_inner(pt_new_inner, new_regions)) == interp_to_l0(tok@, rebuild_root_pt_inner(dir_pt, iset![])));
                    }
                ...
                assert(candidate_mapping_overlaps_existing_vmem(interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat).interp(), vaddr as nat, pte@)
                    <==> candidate_mapping_overlaps_existing_vmem(interp_to_l0(tok@, rebuild_root_pt_inner(dir_pt, iset![])), vaddr as nat, pte@))
                by {
                    let interp_now_inner = interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat);
                    let interp_now_outer = interp_at(tok@, pt, layer as nat, ptr, base as nat);
                    assert(interp_to_l0(tok@, rebuild_root_pt_inner(dir_pt, iset![])) == interp_to_l0(old(tok)@, rebuild_root_pt(pt, iset![])));
        ...
        if x86_arch_exec.entry_size(layer) == pte.frame.size {
            let ghost root_pt = rebuild_root_pt(pt, iset![]);
            ...
            proof {
                ...
                assert(tok_after_write.regions.dom() == tok@.regions.dom().union(iset![]));
                assert(map_builder_pre(tok@, pt, tok_after_write, pt, layer as nat, ptr, iset![]));

                assert(interp_to_l0(tok_after_write, rebuild_root_pt(pt, iset![]))
                            === interp_to_l0(tok@, rebuild_root_pt(pt, iset![])).insert(vaddr as nat, pte@))
                by {
                    let idx = idx as nat;
                    ...
                    assert(pre_interp.interp_of_entry(idx) =~= imap![]);
                    let entry_base = x86_arch_spec.entry_base(layer as nat, base, idx);
                    assert(post_interp.interp_of_entry(idx) =~= imap![entry_base => pte]);
                ...
                assert(inv(old(tok)@, root_pt)) by {
                    assert(map_builder_pre(old(tok)@, pt, old(tok)@, pt, layer as nat, ptr, iset![]));
                };
            ...
            assert(tok@.regions.dom() =~= old(tok)@.regions.dom().union(iset![]));
            assert(pt.used_regions.union(iset![]) =~= pt.used_regions);
            ...
            match map_frame_aux(Tracked(tok), Ghost(new_dir_pt), layer + 1, new_dir_addr, entry_base, vaddr, pte, Ghost(rebuild_root_pt_inner)) {
                Ok(rec_res) => {
                    assert(!candidate_mapping_overlaps_existing_vmem(interp_to_l0(tok_with_empty, rebuild_root_pt_inner(new_dir_pt, iset![])), vaddr as nat, pte@));
                    ...
                    proof {
                        ...
                        assert(interp_to_l0(tok_with_empty, rebuild_root_pt_inner(new_dir_pt, iset![])) == interp_to_l0(old(tok)@, rebuild_root_pt(pt, iset![])));
                        assert(!candidate_mapping_overlaps_existing_vmem(interp_to_l0(old(tok)@, rebuild_root_pt(pt, iset![])), vaddr as nat, pte@));
                ...
                Err(e) => {
                    proof {
                        ...
                        assert(interp_new_dir.interp() =~= imap![]);
...
pub proof fn lemma_zeroed_page_implies_empty_at(tok: WrappedTokenView, pt: PTDir, layer: nat, ptr: usize)
    requires
        ...
        pt.region.base == ptr,
        ptr == pt.region.base,
        pt.used_regions === iset![pt.region],
        layer_in_range(layer),
        pt.entries.len() == X86_NUM_ENTRIES,
...
fn insert_empty_directory(
    ...
    Ghost(rebuild_root_pt): Ghost<spec_fn(PTDir, ISet<MemRegion>) -> PTDir>,
            ...
            PDE, // new_dir_entry
            Ghost<spec_fn(PTDir, ISet<MemRegion>) -> PTDir> // rebuild_root_pt_inner
            ))
    requires
        !old(tok)@.change_made,
        old(tok).inv(),
    ...
    ensures
        ...
        ({ let pt_with_empty = res.0@; let new_dir_region = res.1; let new_dir_entry = res.2;
           ...
           &&& forall|tok_new, pt_new_inner, new_regions|
              ...
              ==> {
                  ...
                  &&& interp_at(tok_new, pt_new_inner, layer as nat + 1, new_dir_region.base, entry_base as nat).interp()
                       ...
                       ==> interp_to_l0(tok_new, rebuild_root_pt_inner(pt_new_inner, new_regions))
                               == interp_to_l0(final(tok)@, rebuild_root_pt_inner(new_dir_pt, iset![]))
                  &&& interp_at(tok_new, pt_new_inner, layer as nat + 1, new_dir_region.base, entry_base as nat).interp()
                          === interp_at(final(tok)@, new_dir_pt, layer as nat + 1, new_dir_region.base, entry_base as nat).interp().insert(vaddr as nat, pte@)
                       ==> interp_to_l0(tok_new, rebuild_root_pt_inner(pt_new_inner, new_regions))
                               === interp_to_l0(final(tok)@, rebuild_root_pt_inner(new_dir_pt, iset![])).insert(vaddr as nat, pte@)
           }
           &&& interp_to_l0(final(tok)@, rebuild_root_pt_inner(new_dir_pt, iset![])) == interp_to_l0(old(tok)@, rebuild_root_pt(pt, iset![]))
           &&& candidate_mapping_overlaps_existing_vmem(interp_at(final(tok)@, new_dir_pt, layer as nat + 1, new_dir_region.base, entry_base as nat).interp(), vaddr as nat, pte@)
               <==> candidate_mapping_overlaps_existing_vmem(interp_to_l0(final(tok)@, rebuild_root_pt_inner(new_dir_pt, iset![])), vaddr as nat, pte@)
        }),
{
    broadcast use
        lemma_union_empty,
    ...
    let ghost root_pt0 = rebuild_root_pt(pt, iset![]);
    ...
    let ghost new_dir_pt =
        PTDir {
            ...
            used_regions: iset![new_dir_region@],
        };
    ...
    assert(tok_with_alloc.regions.dom() =~= old(tok)@.regions.dom().union(iset![new_dir_region@]));
    ...
    let ghost root_pt1 = rebuild_root_pt(pt, iset![new_dir_region@]);
    //assert(pt.used_regions =~= pt.used_regions.union(iset![]));
    assert(map_builder_pre(old(tok)@, pt, tok_with_alloc, pt, layer as nat, ptr, iset![new_dir_region@]));
    ...
    proof {
        ...
        assert(old_interp_at_idx.interp(entry_base) === imap![]);
        ...
        assert(tok_with_empty.regions.dom() =~= old(tok)@.regions.dom().union(iset![new_dir_region@]));
        assert(pt_with_empty.used_regions =~= pt.used_regions.union(iset![new_dir_region@]));
    ...
    let ghost root_pt2 = rebuild_root_pt(pt_with_empty, iset![new_dir_region@]);
    //assert(tok_with_alloc.regions =~= old(tok)@.regions);

    assert(map_builder_pre(old(tok)@, pt, tok_with_empty, pt_with_empty, layer as nat, ptr, iset![new_dir_region@]));
    ...
    let ghost rebuild_root_pt_inner = |pt_new_inner: PTDir, new_regions: ISet<MemRegion>| {
        let all_new_regions = new_regions.union(iset![new_dir_region@]);
        let pt_new = PTDir {
            entries: pt.entries.update(idx as int, Some(pt_new_inner)),
    ...
    assert(pt_with_empty.entries =~= PTDir {
        entries: pt.entries.update(idx as int, Some(new_dir_pt)),
        used_regions: pt.used_regions.union(iset![new_dir_region@]),
        ..pt
    }.entries);
    //assert(iset![].union(iset![new_dir_region@]) =~= iset![new_dir_region@]);
    assert(rebuild_root_pt_inner(new_dir_pt, iset![]) =~= rebuild_root_pt(pt_with_empty, iset![new_dir_region@])) by {
        let all_new_regions = iset![].union(iset![new_dir_region@]);
        let pt_new = PTDir {
            entries: pt.entries.update(idx as int, Some(new_dir_pt)),
            used_regions: pt.used_regions.union(all_new_regions),
            ..pt
        ...
        assert(all_new_regions =~= iset![new_dir_region@]);
        assert(rebuild_root_pt_inner(new_dir_pt, iset![]) == rebuild_root_pt(pt_new, all_new_regions));
    };
    ...
    assert forall|tok_new, pt_new_inner, new_regions|
       ...
       implies {
           ...
           &&& interp_at(tok_new, pt_new_inner, layer as nat + 1, new_dir_addr, entry_base as nat).interp()
                ...
                ==> interp_to_l0(tok_new, rebuild_root_pt_inner(pt_new_inner, new_regions))
                        == interp_to_l0(tok_with_empty, rebuild_root_pt_inner(new_dir_pt, iset![]))
           &&& interp_at(tok_new, pt_new_inner, layer as nat + 1, new_dir_addr, entry_base as nat).interp()
                   === interp_at(tok_with_empty, new_dir_pt, layer as nat + 1, new_dir_addr, entry_base as nat).interp().insert(vaddr as nat, pte@)
                ==> interp_to_l0(tok_new, rebuild_root_pt_inner(pt_new_inner, new_regions))
                        === interp_to_l0(tok_with_empty, rebuild_root_pt_inner(new_dir_pt, iset![])).insert(vaddr as nat, pte@)
       }
    by {
        let all_new_regions = new_regions.union(iset![new_dir_region@]);
        ...
        assert(pt.used_regions.union(iset![new_dir_region@]).union(new_regions) =~= pt.used_regions.union(all_new_regions));
    ...
    assert(candidate_mapping_overlaps_existing_vmem(interp_at(tok_with_empty, new_dir_pt, layer as nat + 1, new_dir_region.base, entry_base as nat).interp(), vaddr as nat, pte@)
               <==> candidate_mapping_overlaps_existing_vmem(interp_to_l0(tok_with_empty, rebuild_root_pt_inner(new_dir_pt, iset![])), vaddr as nat, pte@))
    by {
...
spec fn unmap_builder_pre(tok_old: WrappedTokenView, pt_old: PTDir, tok_new: WrappedTokenView, pt_new: PTDir, layer: nat, ptr: usize, removed_regions: ISet<MemRegion>) -> bool {
    &&& tok_new.pt_mem.pml4 === tok_old.pt_mem.pml4
               ...
               ==> {
                   ...
                   &&& interp_at(tok_new, pt_new, layer as nat, ptr, base as nat).interp()
                        ...
                        ==> interp_to_l0(tok_new, rebuild_root_pt(pt_new, removed_regions))
                                === interp_to_l0(old(tok)@, rebuild_root_pt(pt, iset![])).remove(vaddr as nat)
        }
    ensures
        final(tok).inv(),
        final(tok)@.pt_mem.pml4 == old(tok)@.pt_mem.pml4,
    ...
    decreases X86_NUM_LAYERS - layer
{
    ...
    assert(PT::inv(old(tok)@, root_pt)) by {
        assert(unmap_builder_pre(old(tok)@, pt, old(tok)@, pt, layer as nat, ptr, iset![]));
    };
    let idx: usize = x86_arch_exec.index_for_vaddr(layer, base, vaddr);
    proof { indexing::lemma_index_from_base_and_addr(base as nat, vaddr as nat, x86_arch_spec.entry_size(layer as nat), X86_NUM_ENTRIES as nat); }
    let entry = entry_at_unmap(Tracked(tok), Ghost(pt), layer, ptr, idx);
    ...
    if entry.is_present() {
        if entry.is_dir(layer) {
            ...
            let ghost rebuild_root_pt_inner = |pt_new_inner: PTDir, removed_regions: ISet<MemRegion>| {
                let pt_new = PTDir {
                    entries: pt.entries.update(idx as int, Some(pt_new_inner)),
            ...
            assert(rebuild_root_pt_inner(dir_pt, iset![]) == rebuild_root_pt(pt, iset![])) by {
                assert(pt.entries =~= PTDir {
                    entries: pt.entries.update(idx as int, Some(dir_pt)),
                    used_regions: pt.used_regions.union(iset![]),
                    ..pt
                   ...
                   implies {
                       ...
                       &&& interp_at(tok_new, pt_new_inner, layer as nat + 1, dir_addr, entry_base as nat).interp()
                            ...
                            ==> interp_to_l0(tok_new, rebuild_root_pt_inner(pt_new_inner, removed_regions))
                                    === interp_to_l0(tok@, rebuild_root_pt_inner(dir_pt, iset![])).remove(vaddr as nat)
            } by {
                assert(inv_at(tok_new, pt_new_inner, (layer + 1) as nat, dir_addr));
                let pt_new = PTDir {
                    region: pt.region,
            ...
            assert(interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat).interp().contains_key(vaddr as nat)
                <==> interp_to_l0(tok@, rebuild_root_pt_inner(dir_pt, iset![])).contains_key(vaddr as nat))
            by {
                ...
                if interp_to_l0(tok@, rebuild_root_pt_inner(dir_pt, iset![])).contains_key(vaddr as nat) {
                    assert(interp_outer.interp().contains_key(vaddr as nat));
            ...
            if aligned_exec(vaddr, x86_arch_exec.entry_size(layer)) {
                ...
                proof {
                    ...
                    assert(unmap_builder_pre(old(tok)@, pt, tok_after_write, pt, layer as nat, ptr, iset![])) by {
                        assert(tok_after_write.regions.dom() == old(tok)@.regions.dom().difference(iset![]));
                    };
                    ...
                    by {
                        ...
                        assert(interp_old.interp_of_entry(idx as nat).dom() == iset![vaddr as nat]);
                    ...
                    by {
                        assert(unmap_builder_pre(old(tok)@, pt, tok_after_write, pt, layer as nat, ptr, iset![]));
                    };
                }

                WrappedUnmapToken::write_change(Tracked(tok), ptr, idx, 0usize, Ghost(pt.region), Ghost(root_pt));

                let ghost removed_regions = ISet::empty();
                ...
                *frame = unmapped_region;
                ...
                proof {
                    assert(entry_base != vaddr);
                    let interp_old = interp_at(old(tok)@, pt, layer as nat, ptr, base as nat);
                    assert(interp_old.interp_of_entry(idx as nat).dom() == iset![entry_base as nat]);
        ...
        proof {
            let interp_old = interp_at(old(tok)@, pt, layer as nat, ptr, base as nat);
            assert(interp_old.interp_of_entry(idx as nat).dom() === iset![]);
            assert_by_contradiction!(!interp_old.interp().contains_key(vaddr as nat), {
                interp_old.lemma_interp_contains_key_implies_interp_of_entry_contains_key_at_index(vaddr as nat);
            });
        }
...
spec fn protect_framing_pre(tok_old: WrappedTokenView, tok_new: WrappedTokenView, pt: PTDir, layer: nat, ptr: usize) -> bool {
    &&& tok_new.pt_mem.pml4 === tok_old.pt_mem.pml4
    &&& tok_new.regions.dom() == tok_old.regions.dom()
    &&& inv_at(tok_new, pt, layer as nat, ptr)
    &&& forall|r: MemRegion| !pt.used_regions.contains(r) ==> #[trigger] tok_new.regions[r] === tok_old.regions[r]
}
...
fn protect_aux(
    ...
    Ghost(pt): Ghost<PTDir>, // PTDir for the current subtree of the page table
    Ghost(root_pt): Ghost<PTDir>, // PTDir for the whole page table
    layer: usize,
    ptr: usize,
    base: usize,
    vaddr: usize,
    ...
    requires
        old(tok).inv(),
        !old(tok)@.change_made,
        old(tok)@.args == (OpArgs::Protect { base: vaddr, flags: *permissions, pkey: pkey as nat }),
        inv_at(old(tok)@, pt, layer as nat, ptr),
        no_empty_directories(old(tok)@, pt, layer as nat, ptr),
        ...
        interp_at(old(tok)@, pt, layer as nat, ptr, base as nat).interp().contains_key(vaddr as nat)
             ==> interp_to_l0(old(tok)@, root_pt)[vaddr as nat]
                 == interp_at(old(tok)@, pt, layer as nat, ptr, base as nat).interp()[vaddr as nat],
        // The changes we make in protect_aux don't affect the PTDir, so unlike with unmap and
        // map_frame, we just pass the root PTDir in as an argument and don't need a closure to
        // rebuild it.
        forall|tok_new|
           #[trigger] protect_framing_pre(old(tok)@, tok_new, pt, layer as nat, ptr)
               ==> {
                   &&& inv(tok_new, root_pt)
                   &&& forall|new_pte|
                       interp_at(tok_new, pt, layer as nat, ptr, base as nat).interp()
                           === interp_at(old(tok)@, pt, layer as nat, ptr, base as nat).interp().insert(vaddr as nat, new_pte)
                        ==> interp_to_l0(tok_new, root_pt)
    ...
    decreases X86_NUM_LAYERS - layer
{
    ...
    assert(PT::inv(old(tok)@, root_pt)) by {
        assert(protect_framing_pre(old(tok)@, old(tok)@, pt, layer as nat, ptr));
    };
    let idx: usize = x86_arch_exec.index_for_vaddr(layer, base, vaddr);
    proof { indexing::lemma_index_from_base_and_addr(base as nat, vaddr as nat, x86_arch_spec.entry_size(layer as nat), X86_NUM_ENTRIES as nat); }
    let entry = entry_at_protect(Tracked(tok), Ghost(pt), layer, ptr, idx);
    let entry_base: usize = x86_arch_exec.entry_base(layer, base, idx);
    proof { indexing::lemma_entry_base_from_index(base as nat, idx as nat, x86_arch_spec.entry_size(layer as nat)); }
    assert(interp_at_entry(tok@, pt, layer as nat, ptr, base as nat, idx as nat)
           == interp_at(tok@, pt, layer as nat, ptr, base as nat).entries[idx as int]);
    if entry.is_present() {
        if entry.is_dir(layer) {
            ...
            assert forall|tok_new| #[trigger] protect_framing_pre(tok@, tok_new, dir_pt, layer as nat + 1, dir_addr)
                implies {
                   &&& inv(tok_new, root_pt)
                   &&& forall|new_pte|
                       interp_at(tok_new, dir_pt, layer as nat + 1, dir_addr, entry_base as nat).interp()
                           === interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat).interp().insert(vaddr as nat, new_pte)
                        ==> interp_to_l0(tok_new, root_pt)
                            === #[trigger] interp_to_l0(tok@, root_pt).insert(vaddr as nat, new_pte)
            } by {
                assert(inv_at(tok_new, dir_pt, (layer + 1) as nat, dir_addr));

                // None of the entries at this level change
                assert forall|i: nat| i < X86_NUM_ENTRIES implies
                    #[trigger] entry_at_spec(tok_new, pt, layer as nat, ptr, i) == entry_at_spec(old(tok)@, pt, layer as nat, ptr, i) by { };

                assert(inv_at(tok_new, pt, layer as nat, ptr)) by {
                    reveal(ghost_pt_used_regions_pairwise_disjoint);
                    lemma_directories_obey_invariant_at_framing(tok@, pt, tok_new, pt, layer as nat, ptr, idx as nat);
                };

                assert(protect_framing_pre(tok@, tok_new, pt, layer as nat, ptr));
                assert(inv(tok_new, root_pt));

                let interp_now_inner = interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat);
                let interp_new_inner = interp_at(tok_new, dir_pt, layer as nat + 1, dir_addr, entry_base as nat);
                let interp_old = interp_at(old(tok)@, pt, layer as nat, ptr, base as nat);
                let interp_new = interp_at(tok_new, pt, layer as nat, ptr, base as nat);

                assert forall|new_pte|
                    interp_at(tok_new, dir_pt, layer as nat + 1, dir_addr, entry_base as nat).interp()
                        === interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat).interp().insert(vaddr as nat, new_pte)
                implies interp_to_l0(tok_new, root_pt) === #[trigger] interp_to_l0(tok@, root_pt).insert(vaddr as nat, new_pte)
                by {
                    // Prove the interp property for the new builder
                    if interp_new_inner.interp() === interp_now_inner.interp().insert(vaddr as nat, new_pte) {
                        lemma_interp_at_aux_facts(tok_new, pt, layer as nat, ptr, base as nat, seq![]);
                        assert(interp_new.interp() === interp_old.interp().insert(vaddr as nat, new_pte)) by {
                            assert forall|i: nat| i < X86_NUM_ENTRIES && i != idx implies
                                interp_at(tok_new, pt, layer as nat, ptr, base as nat).entries[i as int]
                                === #[trigger] interp_at(old(tok)@, pt, layer as nat, ptr, base as nat).entries[i as int]
                            by {
                                reveal(ghost_pt_used_regions_pairwise_disjoint);
                                lemma_interp_at_entry_changed_tok(old(tok)@, pt, tok_new, pt, layer as nat, ptr, base as nat, i);
                            };
                            assert(interp_new.entries[idx as int].interp(entry_base as nat)
                                === interp_old.entries[idx as int].interp(entry_base as nat).insert(vaddr as nat, new_pte));
                            assert(interp_new.entries == interp_old.entries.update(idx as int, l1::NodeEntry::Directory(interp_new_inner)));

                            interp_old.lemma_interp_entries_insert_implies_interp_insert(idx as nat, vaddr as nat, l1::NodeEntry::Directory(interp_new_inner), new_pte);
                        };
                        assert(interp_to_l0(tok_new, root_pt) === interp_to_l0(tok@, root_pt).insert(vaddr as nat, new_pte));
                    }
                };
            };

            assert(interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat).interp().contains_key(vaddr as nat)
                <==> interp_to_l0(tok@, root_pt).contains_key(vaddr as nat))
            by {
                let interp_inner = interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat);
                let interp_outer = interp_at(tok@, pt, layer as nat, ptr, base as nat);
                ...
                if interp_to_l0(tok@, root_pt).contains_key(vaddr as nat) {
                    assert(interp_outer.interp().contains_key(vaddr as nat));
                    interp_outer.lemma_interp_contains_key_implies_interp_of_entry_contains_key_at_index(vaddr as nat);
                    assert(interp_inner.interp().contains_key(vaddr as nat));
                }
            ...
            assert(interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat).interp().contains_key(vaddr as nat)
                ==> interp_to_l0(tok@, root_pt)[vaddr as nat]
                    == interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat).interp()[vaddr as nat])
            by {
                let interp_inner = interp_at(tok@, dir_pt, layer as nat + 1, dir_addr, entry_base as nat);
                let interp_outer = interp_at(tok@, pt, layer as nat, ptr, base as nat);
                ...
                if interp_inner.interp().contains_key(vaddr as nat) {
                    interp_outer.lemma_interp_of_entry_contains_mapping_implies_interp_contains_mapping(idx as nat);
                    assert(interp_outer.interp_of_entry(idx as nat).contains_key(vaddr as nat));
                    assert(interp_outer.interp().contains_pair(vaddr as nat, interp_inner.interp()[vaddr as nat]));
                }
            };
            ...
            match protect_aux(Tracked(tok), Ghost(dir_pt), Ghost(root_pt), layer + 1, dir_addr, entry_base, vaddr, permissions, pkey) {
                Ok(_) => {
                    proof {
                        assert(forall|i: nat| i < X86_NUM_ENTRIES && i != idx
                            ==> #[trigger] entry_at_spec(tok@, pt, layer as nat, ptr, i)
                                == entry_at_spec(old(tok)@, pt, layer as nat, ptr, i));

                        assert(entry_at_spec(tok@, pt, layer as nat, ptr, idx as nat)@ is Directory);

                        assert(inv_at(tok@, dir_pt, (layer + 1) as nat, dir_addr));
                        ...
                        // postconditions
                        assert(forall|r: MemRegion| !(#[trigger] pt.used_regions.contains(r))
                            ==> tok@.regions[r] === old(tok)@.regions[r]);
                        assert(tok@.regions.dom() =~= old(tok)@.regions.dom());
                        assert(!empty_at(tok@, pt, layer as nat, ptr)) by {
                            reveal(ghost_pt_used_regions_pairwise_disjoint);
                        };
                        assert(no_empty_directories(tok@, pt, layer as nat, ptr)) by {
                            reveal(ghost_pt_used_regions_pairwise_disjoint);
                            assert(entry_at_spec(tok@, pt, layer as nat, ptr, idx as nat)@ matches GPDE::Directory { addr, ..} && addr == dir_addr);
                            assert(!empty_at(tok@, pt.entries[idx as int]->Some_0, layer as nat + 1, dir_addr));
                            assert(no_empty_directories(tok@, pt.entries[idx as int]->Some_0, layer as nat + 1, dir_addr));
                            lemma_no_empty_directories_framing(old(tok)@, pt, tok@, pt, layer as nat, ptr, base as nat, idx as nat);
                        };
                    }
                    Ok(())
            ...
            assert(interp_at(old(tok)@, pt, layer as nat, ptr, base as nat).interp_of_entry(idx as nat).dom() == iset![entry_base as nat]);
            assert(interp_at(old(tok)@, pt, layer as nat, ptr, base as nat).entries[idx as int] is Page);
            assert(entry_at_spec(tok@, pt, layer as nat, ptr, idx as nat)@ is Page);
            if aligned_exec(vaddr, x86_arch_exec.entry_size(layer)) {
                let new_entry = entry.change_page_permissions(permissions, pkey);

                proof {
                    let tok_after_write = old(tok)@.write(idx, new_entry.entry, pt.region, true);
                    assert(tok@.read(idx, pt.region) == entry.entry);
                    assert(tok_after_write.read(idx, pt.region) == new_entry.entry & MASK_NEG_DIRTY_ACCESS);
                    assert(entry.entry == new_entry.entry & MASK_NEG_DIRTY_ACCESS ==> entry@ == new_entry@) by {
                        broadcast use PDE::lemma_view_unchanged_dirty_access;
                        assert(forall|a: usize, b: usize, m: usize| a & m == b ==> a & m == b & m) by (bit_vector);
                    };
                    assert(entry_at_spec(tok_after_write, pt, layer as nat, ptr, idx as nat)@ == new_entry@) by {
                        broadcast use PDE::lemma_view_unchanged_dirty_access;
                        assert(forall|a: usize, b: usize, m: usize| a & m == b ==> a & m == b & m) by (bit_vector);
                    };
                    assert(forall|a: usize| a & MASK_NEG_DIRTY_ACCESS & MASK_NEG_PROT_ALL
                        == a & MASK_NEG_PROT_ALL & MASK_NEG_DIRTY_ACCESS) by (bit_vector);
                    assert(tok@.read(idx, pt.region) & MASK_NEG_DIRTY_ACCESS == tok@.read(idx, pt.region)) by {
                        assert(forall|a: usize, b: usize| a & b & b == a & b) by (bit_vector);
                    };
                    assert(tok@.read(idx, pt.region) == entry.entry & MASK_NEG_DIRTY_ACCESS);
                    assert(pages_have_bit7(tok@, pt, layer as nat, ptr));

                    assert(entry.entry & bit!(7usize) == bit!(7usize));

                    interp_at(tok@, pt, layer as nat, ptr, base as nat)
                        .lemma_interp_of_entry_contains_mapping_implies_interp_contains_mapping(idx as nat);
                    assert(PT::interp_to_l0(tok@, root_pt).contains_key(tok@.args->Protect_base as nat));

                    assert(entry_at_spec(tok_after_write, pt, layer as nat, ptr, idx as nat)@ is Page);
                    assert forall|i: nat| #![auto] i < X86_NUM_ENTRIES implies entry_at_spec(tok_after_write, pt, layer as nat, ptr, i).hp_pat_is_zero() by {
                        assert(forall|a: usize| a & MASK_PG_FLAG_PAT ==  #[trigger] (a & MASK_NEG_DIRTY_ACCESS) & MASK_PG_FLAG_PAT) by (bit_vector);
                        if i != idx {
                            assert(entry_at_spec(tok_after_write, pt, layer as nat, ptr, i) == entry_at_spec(tok@, pt, layer as nat, ptr, i));
                        }
                    };
                    assert(forall|i: nat| i < X86_NUM_ENTRIES && i != idx ==>
                        #[trigger] entry_at_spec(tok_after_write, pt, layer as nat, ptr, i)
                    ...
                    assert(protect_framing_pre(old(tok)@, tok_after_write, pt, layer as nat, ptr)) by {
                        assert(tok_after_write.regions.dom() == old(tok)@.regions.dom());
                    };
                    assert(PT::inv(tok_after_write, root_pt));

                    let old_pte = interp_at_entry(tok@, pt, layer as nat, ptr, base as nat, idx as nat)->Page_0;
                    let new_pte = PTE { frame: old_pte.frame, flags: *permissions,  pkey: pkey as nat };
                    ...
                    assert(interp_to_l0(tok_after_write, root_pt) === interp_to_l0(tok@, root_pt).insert(vaddr as nat, new_pte)) by {
                        let pre_interp = interp_at(tok@, pt, layer as nat, ptr, base as nat);
                        let post_interp = interp_at(tok_after_write, pt, layer as nat, ptr, base as nat);
                        assert(interp_at_entry(tok_after_write, pt, layer as nat, ptr, base as nat, idx as nat) === l1::NodeEntry::Page(new_pte));
                        assert(pre_interp.inv());
                        assert(post_interp.inv());
                        assert_seqs_equal!(post_interp.entries == pre_interp.entries.update(idx as int, l1::NodeEntry::Page(new_pte)), j => {
                            lemma_interp_at_facts(tok@, pt, layer as nat, ptr, base as nat);
                            lemma_interp_at_facts(tok_after_write, pt, layer as nat, ptr, base as nat);
                            if j == idx {
                            } else {
                                broadcast use lemma_interp_at_entry_changed_tok;
                                assert(interp_at_entry(tok_after_write, pt, layer as nat, ptr, base as nat, j as nat)
                                    == interp_at_entry(tok@, pt, layer as nat, ptr, base as nat, j as nat));
                            }
                        });
                        assert(pre_interp.interp_of_entry(idx as nat).insert(vaddr as nat, new_pte)
                            == pre_interp.update(idx as nat, l1::NodeEntry::Page(new_pte)).interp_of_entry(idx as nat));
                        pre_interp.lemma_interp_of_entry_insert_page_implies_interp_insert_page(idx as nat, vaddr as nat, new_pte);
                        assert(post_interp.interp() === pre_interp.interp().insert(vaddr as nat, new_pte));
                    };

                    assert(interp_at(old(tok)@, pt, layer as nat, ptr, base as nat).interp().contains_key(vaddr as nat));
                    assert(interp_to_l0(old(tok)@, root_pt)[vaddr as nat] == interp_at(old(tok)@, pt, layer as nat, ptr, base as nat).interp()[vaddr as nat]);
                }

                WrappedProtectToken::write_change(Tracked(tok), ptr, idx, new_entry.entry, Ghost(pt.region), Ghost(root_pt));
...
pub fn protect(Tracked(tok): Tracked<&mut WrappedProtectToken>, pt: &mut Ghost<PTDir>, pml4: usize, vaddr: usize, permissions: &Flags, pkey: u8) -> (res: Result<(),()>)
    requires
        !old(tok)@.change_made,
        inv_and_nonempty(old(tok)@, old(pt)@),
        old(tok).inv(),
    ...
    ensures
        ...
        final(tok)@.orig_st.mmu@.cr3 == old(tok)@.orig_st.mmu@.cr3,
{
    if let Ok(_) = protect_aux(Tracked(tok), *pt, *pt, 0, pml4, 0, vaddr, permissions, pkey) {
